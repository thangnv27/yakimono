...
$('document').ready(function($) {
	...
	*/
	...
	if ($("#map_canvas").length) {
		...
		if (markerImage !== '') {
			markerOptions.icon = { url: markerImage, origin: new google.maps.Point(0, 0), anchor: new google.maps.Point(0, 0) };
		}
	...
	*/
	...
	$(".button-search").on("click", function(e) {
		...
		geocoder.geocode({'address': address}, function(results, status) {
			if (status == google.maps.GeocoderStatus.OK) {
				...
				var markerOptions = { position: latLng, title: address, map: map, draggable: true };
				...
				if (markerImage !== '') {
					markerOptions.icon = { url: markerImage, origin: new google.maps.Point(0, 0), anchor: new google.maps.Point(2 + parseInt(markerWidth/2, 10), parseInt(markerHeight, 10)) - 5 };
				}
	...
	$(".mulitiple_map_canvas").each(function() {
		...
		if (markerImage !== '') {
			var orginPoint = new google.maps.Point(0, 0);
			var anchorPoint = new google.maps.Point(2 + parseInt(markerWidth/2, 10), parseInt(markerHeight, 10) - 5);
	...
	var myRecords = [
		{
			...
			'hour': '12:30',
			'name': 'Weezer',
			'email': 'wwwwweee@zer.com',
			'phone': '+48 123 123 123',
			'amount': '12',
			'custom1': 'custooom 1',
			'custom2': 'custooom 2',
			'custom3': 'custooom 3',
			'message': 'meeeesage in here !',
			'confirmed': false,
			'notes': ''
		},
		{
			...
			'custom2': 'custooom 2',
			'custom3': 'custooom 3',
			'message': 'meeeesage in hereeee !!!!! 2',
			'confirmed': true,
			'notes': 'big guest!'
		}
	];

	//$('#yopress-wrap').css('maxWidth', '1200px');
	/*
	$('#reservation-log-table').dynatable({
		dataset: {
			records: myRecords
		}
	});

	$('#add-range').dateRangePicker({showShortcuts: false, startOfWeek: 'monday'})
